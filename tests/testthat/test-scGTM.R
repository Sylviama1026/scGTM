library(scGTM)


test_that("scGTM works", {
  data("df")
  res1 <- scGTM::scGTM(t=df$Time, y1=df$Gene1)
  expect_equal(length(res1), 24)

  res2 <- scGTM::scGTM(t=df$Time, y1=df$Gene1, marginal="Poisson")
  expect_equal(length(res2), 24)

  res3 <- scGTM::runscGTM(t=df$Time, y=df[,3:5])
  expect_equal(dim(res3)[1], 3)

  res4 <- scGTM::plot_result(para=c(2.29,3.27,11.79,0.58,30.4,60.82),
                             t=df$Time,
                             color=c('red', 'blue', 'orange', 'darkgreen'),
                             marginal="ZIP",
                             flag=FALSE,
                             y1=df$Gene1,
                             gene_name="Gene1")
  expect_equal(class(res4)[1], "gg")
})
